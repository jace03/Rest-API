...
}catch(PDOException $ex){               //connect to database and if fail what the response is
...
if(array_key_exists("taskid", $_GET)){
    ...
    elseif($_SERVER['REQUEST_METHOD'] === 'DELETE'){
        try {
            ...
            if($rowCount === 0) {//if it hasn't deleted anything comes out task not found
                $response = new Response();
                $response->setHttpStatusCode(404);
                $response->setSuccess(false);
                $response->addMessage("Task not found");
            ...
            $response = new Response();//if it has deleted something it comes out a status back to the client
            $response->setHttpStatusCode(200);
            $response->setSuccess("Task deleted");
            $response->send();
            exit;

        }catch(PDOException $ex){
            $respnose = new Response();
            $response->setHttpStatusCode(500);
            $response->setSuccess(false);
    ...
    elseif($_SERVER['REQUEST_METHOD'] === 'PATCH'){
        try{
            ...
            //update tblask set title = :title, description = :description where id = taskid
            ...
            $queryFields = rtrim($queryFields, ", ");//strips the final ," " for the sql statement

            if($title_updated === false && $description_updated === false && $completed_updated === false){
                $response = new Response();
                $response ->setHttpStatusCode(400);
                $response->setSuccess(false);
            ...
            $queryString = "update tbltasks set".$queryFields." where id = : tasksid";
            $query = $writeDB->prepare($queryString);

            if($title_updated === true){
                $task->setTitle($jsonData->title);
                $up_title = $task->getTitle();
                $qurey->bindParam(':title', $up_title);
            }
            ...
            if($deadline_updated === true){
                $task->setDescription($jsonData->description);
                $up_description = $task->getDeadline();
            ...
            $query = $writeDB->prepare("select id, title, description, DATE_FORMAT(deadline, '%d/%m/%Y %H:%i') as deadline, $completed from tbltasks where id = :tbltasks");
            $query->bindParam('taskid:',$taskid, PDO::PARAM_INT);
            $query->execute();
    ...
    if($completed != 'Y' && $completed != 'N'){
        ...
        $response->setHttpStatusCode(400);
        $response ->setSuccess(false);
        $response->addMessage('Filter must be Y or N');
        $response->send();
        exit();
    }
    if($_SERVER['REQUEST_METHOD'] === 'GET'){
...
elseif(array_key_exists("page",$_GET)){
    if($_SERVER['REQUEST_METHOD'] === 'GET'){
        ...
        try{
            ...
            $offset = ($page == 1 ? 0 : ($limitPerPage * ($page-1)));//very confusing GET-get all tasks 18:30
    ...
    if($_SERVER["REQUEST_METHOD"] ==='GET') {
        ...
        try{
            if($_SERVER['CONTENT_TYPE'] !== 'application/json'){
                ...
                $response->setHttpStatusCode(400);
                $response->setSuccess(false);
                $response->addMessage("content type not json");
                $response->send();
                exit;
            }
