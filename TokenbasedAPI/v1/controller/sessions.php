...
if(array_key_exists("sessionid", $_GET)){
    ...
    elseif ($_SERVER['REQUEST_METHOD'] === 'PATCH') {
        ...
        try {
            ...
            $returnData['access_token_expires'] = $access_token_expiry_seconds;
            $returnData['refresh_token'] = $refreshtoken;
            $returnData['refresh_token_expires'] = $refresh_token_expiry_seconds;
...
elseif(empty($_GET)){
    ...
    sleep(1);//delay 1 second to slow hackers
    ...
    if(strlen($jsonData->username) < 1 || strlen($jsonData->username) > 255 || strlen($jsonData->password) <1 || strlen($jsonData->password) > 255)  {
        ...
        (!isset($jsonData->username) > 255 ? $response->addMessage("Username must be less than 255 characters") : false);
        (!isset($jsonData->password) < 1 ? $response->addMessage("Password cannot be blank") : false);
        (!isset($jsonData->password) > 255 ? $response->addMessage("Password must be less than 255 characters") : false);
        $response->send();
        exit;
    }
